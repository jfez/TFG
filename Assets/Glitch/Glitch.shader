...
#include "UnityCG.cginc"
	...
	half4 frag(v2f i) : COLOR
	{
		...
		float4 normal = tex2D(_dispTex, uv.xy * scale);
		...
		float4 sumR = tex2D(_MainTex, uvR);
		float4 sumG = tex2D(_MainTex, uvG);
		float4 sumB = tex2D(_MainTex, uvB);
#ifdef INVERT
		return fixed4(1 - sumR.r, sumG.g, sumB.b, 1);
#else
		return fixed4(sumR.r, sumG.g, sumB.b, 1);
#endif
	}
